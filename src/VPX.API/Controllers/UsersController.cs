...
namespace VPX.API.Controllers
{
    ...
    public class UsersController : ControllerBase
    {
        ...
        {
            var profile = await userRolesService.AddRole(model.Id, Role.Teacher);
            return Ok(profile);
        }
        ...
        {
            var profile = await userRolesService.RemoveRole(model.Id, Role.Teacher);
            return Ok(profile);
        }
        ...
        {
            var profile = await userRolesService.AddRole(model.Id, Role.Student);
            return Ok(profile);
        }
        ...
        {
            var profile = await userRolesService.RemoveRole(model.Id, Role.Student);
            return Ok(profile);
        }
